...
trước đó của P1-C (xem git history nếu cần đối chiếu bản cũ):
  (a) Quote check: mỗi citation.quote chạy qua `validators.check_quote(quote,
      [text_của_ĐÚNG_chunk_được_cite])` (khớp theo doc_id+section, giữ đúng ngữ nghĩa
  ...
  (c) Số liệu: `validators.check_numbers(text, allowed_sources=[chunk texts])` — MỌI
      con số trong "text" phải truy được về evidence (xem P1-A docstring: substring
...
2. Mỗi ý khuyến nghị chính trong "text" phải có ít nhất 1 citation trỏ đúng về chunk nguồn (đúng doc_id + section), kèm "quote" là đoạn NGUYÊN VĂN chép chính xác (không diễn giải, không đổi từ) tối đa 200 ký tự lấy trực tiếp từ chunk đó.
...
def _validate_citations(raw_citations: list[Any], chunks: list[dict[str, Any]]) -> list[dict[str, str]]:
    """Dùng `validators.check_quote` (P1-A) — quote phải khớp (exact hoặc fuzzy, xem
    validators.py) với text của ĐÚNG chunk được cite (khớp theo doc_id+section, tìm
    qua `_find_chunk`) — sai hoặc rỗng -> loại citation đó (không loại cả câu trả
    lời). So với hậu kiểm exact-substring tự viết ở P1-C, nhánh fuzzy giúp không
    fail oan khi KB có chunk bị lỗi trích xuất PDF nhẹ (thiếu ký tự đầu 1 vài từ)."""
    ...
    for cit in raw_citations:
        ...
        if not result.ok:
            continue
        valid.append(
            {"doc_id": doc_id, "section": section, "url": cit.get("url") or chunk.get("url", ""), "quote": quote}
        )
    return valid
...
def _one_attempt(client, prompt: str, chunks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    allowed_sources = [c.get("text") or "" for c in chunks]
    number_check = validators.check_numbers(text, allowed_sources)
    return {
        "text": text,
        "citations": citations,
        "grounded": grounded,
        "number_violations": number_check.violations,
    }
